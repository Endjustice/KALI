...
def verify_sha(file, expected):
    print(Fore.CYAN + "[INFO]" + Style.RESET_ALL + " Verifying SHA256...")
    result = subprocess.run(f"sha256sum {file}", shell=True, capture_output=True, text=True)
...
def extract_rootfs(file):
    print(Fore.CYAN + "[INFO]" + Style.RESET_ALL + " Extracting rootfs...")
    os.makedirs(ROOTFS_DIR, exist_ok=True)
    run(f"proot --link2symlink tar -xf {file} -C {ROOTFS_DIR} --strip-components=1")
...
def main():
    ...
    print(Fore.CYAN + "[INFO]" + Style.RESET_ALL + f"Downloading {variant} for {arch}...")
    run(f"curl -LO {url}")
    verify_sha(filename, sha)
    extract_rootfs(filename)
    banner()
